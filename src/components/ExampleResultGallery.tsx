    ...
    <div style={{ marginTop: 20, marginBottom: 24 }}>
      ...
      {results.length > 1 && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 12, flexWrap: 'wrap' }}>
          <button
            type="button"
            onClick={() => setActive((active - 1 + results.length) % results.length)}
            aria-label="Previous variation"
            style={{ background: 'transparent', border: 'none', color: 'var(--fade)', cursor: 'pointer', padding: 4, display: 'flex' }}
          >
          ...
          <button
            type="button"
            onClick={() => setActive((active + 1) % results.length)}
          ...
          {current?.note && (
            <span className="mono" style={{ color: 'var(--fade)', fontSize: 11.5 }}>
              {current.note}
            </span>
          )}
        </div>
