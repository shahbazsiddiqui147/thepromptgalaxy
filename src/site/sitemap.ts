...
/** Every URL that should be indexed: home, hubs, indexable category x tool combos and published prompts. */
export async function getSitemapEntries(db: Queryable): Promise<SitemapEntry[]> {
  const [categories, tools, styles, combos, prompts] = await Promise.all([
